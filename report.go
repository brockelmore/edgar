...
func (f financialReport) String() string {
	...
	if err != nil {
		...
		log.Println(f.Ops.CollectedData)
		log.Println(f.Ops.Revenue)
		log.Println(f.Ops.CostOfSales)
		log.Println(f.Ops.GrossMargin)
		log.Println(f.Ops.OpIncome)
		log.Println(f.Ops.OpExpense)
		log.Println(f.Ops.NetIncome)
		log.Println(f.Ops.WAShares)
		log.Println(f.Ops.Dps)
